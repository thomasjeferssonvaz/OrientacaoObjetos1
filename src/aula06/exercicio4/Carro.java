...
public class Carro {
	...
	public void aceleral(int velocidadeIncrementar) {
		if (ligado) {
			if(velocidade <200) {
				System.out.printf("O carro de marca %s, modelo %s e cor %s está acelerando.\n", marca, modelo, cor);
				velocidade = velocidade + velocidadeIncrementar;
	...
	public void frear(int velocidadeDecrementar) {
		
		if(ligado) {
			if(velocidade>=10) {
				System.out.printf("O carro de marca %s, modelo %s e cor %s está freando.\n", marca, modelo, cor);
				velocidade = velocidade - velocidadeDecrementar;
				System.out.println("Velocidade atual do carro: "+ velocidade);
			}
		} else {
